...
flowchart LR
  ...
  pkg_browser["browser"]
  svc_browser["ctx.browser<br/>CDP browser automation registry"]
...
| `ctx.browser` | `seam` | [`browser`](../packages/browser/browser) | [`browser-playwright`](../packages/browser/browser-playwright) | [`tool-browser`](../packages/browser/tool-browser), `apiproxy` | - | 提供方拥有浏览器进程和可选面板；tool-browser 负责稳定的面向模型命令，Host API 将面板作为可选的客户端界面公开。 |
